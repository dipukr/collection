...
static Thread main_thread, dead_thread;
...
void initialiseJavaStack(ExecEnv *ee) {
    char *stack = sysMalloc(java_stack_size);
...
static void *shell(void *args) {
    ...
    self = attachThread(((char **) args)[0], TRUE, &self);
...
void createVMThread(char *name, void (*start)(Thread *)) {
    void **args = sysMalloc(2 * sizeof(void *));
    pthread_t tid;
...
void initialiseMainThread(int stack_size) {
    ...
    INST_DATA(main_ee.thread)[vmData_offset] = (u4) &main_thread;
