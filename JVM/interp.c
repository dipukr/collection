...
#define SHIFT_OP(TYPE, OP, mask, ostack, pc)                          \
{                                                                     \
    int s = *--ostack & mask;                                         \
...
u4 *executeJava() {
    ...
    void *handlers[] = {
        ...
        &&opc227, &&opc228, &&opc229, &&opc230, &&opc231, &&opc232, &&opc233, &&unused, &&opc235,
        &&unused, &&unused, &&opc238, &&opc239, &&unused, &&unused, &&unused, &&opc243, &&opc244,
    ...
    while (TRUE) {
        switch (*pc) {
            ...
            DEF_OPC(OPC_ISHL)
                SHIFT_OP(int, <<, 0x1f, ostack, pc);

            DEF_OPC(OPC_LSHL)
                SHIFT_OP(long long, <<, 0x3f, ostack, pc);
            ...
            DEF_OPC(OPC_IUSHR)
                SHIFT_OP(unsigned int, >>, 0x1f, ostack, pc);

            DEF_OPC(OPC_LUSHR)
                SHIFT_OP(unsigned long long, >>, 0x3f, ostack, pc);
            ...
            DEF_OPC(OPC_INVOKEINTERFACE)
                frame->last_pc = (unsigned char *) pc;
                resolveInterfaceMethod(mb->class, CP_DINDEX(pc));

                if (exceptionOccured0(ee))
                    goto throwException;

                OPCODE_REWRITE(pc, OPC_INVOKEINTERFACE_QUICK);
                DISPATCH(pc)

            DEF_OPC(OPC_INVOKEINTERFACE_QUICK) {
                int mtbl_idx;
                ClassBlock *cb;
                int cache = pc[4];
                arg1 = ostack - pc[3];
                ...
                if ((cache >= cb->imethod_table_size) || (new_mb->class != cb->imethod_table[cache].interface)) {
                    ...
                    if (cache == cb->imethod_table_size)
                        THROW_EXCEPTION("java/lang/IncompatibleClassChangeError", "unimplemented interface");
                ...
                if (mtbl_idx == -1)
                    THROW_EXCEPTION("java/lang/AbstractMethodError", "unimplemented interface method");
            ...
            DEF_OPC(OPC_MULTIANEWARRAY)
                frame->last_pc = (unsigned char *) pc;
                resolveClass(mb->class, CP_DINDEX(pc), *pc == OPC_NEW);

                if (exceptionOccured0(ee))
                    goto throwException;

                OPCODE_REWRITE(pc, (pc[0] + OPC_NEW_QUICK-OPC_NEW));
                DISPATCH(pc)

            DEF_OPC(OPC_NEW_QUICK) {
                Class *class = (Class *) CP_INFO(cp, CP_DINDEX(pc));
                Object *obj;
            ...
            DEF_OPC(OPC_ANEWARRAY_QUICK) {
                Class *class = (Class *) CP_INFO(cp, CP_DINDEX(pc));
                char *name = CLASS_CB(class)->name;
                char ac_name[strlen(name) + 4];
                int count = ostack[-1];
                Class *array_class;
                Object *obj;
                ...
                if ((obj = allocArray(array_class, count, 4)) == NULL)
                    goto throwException;
            ...
            DEF_OPC(OPC_MULTIANEWARRAY_QUICK) {
                Class *class = (Class *) CP_INFO(cp, CP_DINDEX(pc));
                int dim = pc[3];
                Object *ob;
            ...
            invokeMethod: {
                    ...
                    if (new_mb->access_flags & ACC_NATIVE) {
                        ostack = (*(u4 *(*)(Class *, MethodBlock *, u4 *)) new_mb->native_invoker)(
                            new_mb->class, new_mb, arg1);
