...
boolean unix_socket::read(void* buf, size_t size,size_t *size_read)
{ 
    if (state != ss_open) { 
	errcode = not_opened;
	return False;
...
const char *unix_socket::addr() const
{
    sockaddr     sa;
    sockaddr_in  sa_in;
    socklen_t	 sa_len = sizeof ( sa ), sa_in_len = sizeof ( sa_in );

    if ( domain == sock_local_domain )
    {
	getsockname ( fd, &sa, &sa_len );
	return strdup(sa.sa_data);
    } else if ( domain == sock_global_domain )
    {
	getsockname ( fd, (sockaddr *)&sa_in, &sa_in_len );
	return strdup ( inet_ntoa ( sa_in.sin_addr ) );
    }

    return "";
}

int unix_socket::port() const
{
    sockaddr_in  sa_in;
...
const char *unix_socket::remote_addr() const
{
    sockaddr     sa;
    sockaddr_in  sa_in;
    socklen_t	 sa_len = sizeof ( sa ), sa_in_len = sizeof ( sa_in );
    ...
    {
	getpeername ( fd, (sockaddr *)&sa_in, &sa_in_len );
	return strdup ( inet_ntoa ( sa_in.sin_addr ) );
    }
...
int unix_socket::remote_port() const
{
    sockaddr_in  sa_in;
    socklen_t	 sa_in_len = sizeof ( sa_in );
